...
namespace AlzaCareerApiTests
{
    ...
    public class AlzaCareerTests
    {
        ...
        [Test]
        public void InvalidPosition_ShouldReturnNotFoundMessage()
        {
            log.Info("[TEST] Validating invalid job position ID — should return JSON with 'NotFound' message");

            using (var driver = GetVisibleBrowserDriver())
            {
                string invalidUrl = "https://webapi.alza.cz/api/career/v2/positions/invalid-position-id";
                driver.Navigate().GoToUrl(invalidUrl);

                string body = driver.FindElement(By.TagName("pre")).Text;
                ...
                Assert.That(message, Is.EqualTo("NotFound"), "Expected message to be 'NotFound'");
            }
        }
